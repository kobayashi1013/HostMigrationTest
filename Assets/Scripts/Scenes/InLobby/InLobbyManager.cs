...
namespace Scenes.InLobby
{
    [RequireComponent(typeof(ObjectToken))]
    public class InLobbyManager : NetworkBehaviour, ISceneManager
    {
        [Header("Prefabs")]
        ...
        public override void Render()
        {
            if (Runner.IsServer)
            {
                ...
                {
                    NetworkManager.Runner.LoadScene(SceneRef.FromIndex((int)SceneName.InGame));
                }
            }
        }
        ...
        public NetworkObject SpawnPlayer(NetworkRunner runner, PlayerRef player)
        {
            ...
            var playerObj = runner.Spawn(
                ...
                (_, obj) =>
                {
                    ...
                    var objectTokenCs = obj.GetComponent<ObjectToken>();
